// this is code from http://wsn.spaceflight.esa.int/iss/index_portal.php
...
function printData() {
    ...
    if (!isDocumentInFullScreenMode()) setLatLon(SGP4.Latitude, SGP4.Longitude);
}
...
function issTrackerResize() {
    ...
    var ref = document.getElementById('isst_dt');
    ref.style.height = (isst_map_w * 0.0624) + "px";
    var ref = document.getElementById('waitmsg');
    ref.style.height = (isst_map_w * 0.0624) + "px";
    ...
    var b = isDocumentInFullScreenMode();
    ref.style.display = (b) ? 'none' : 'block';
    var l = 0;
    if (SGP4.kUnits != 1.0) l = isst_map_h * 0.04;
    document.getElementById('btn_metric2').style.left = l + "px";
    var hh = ((isDocumentInFullScreenMode()) ? ((screen.height - isst_map_w / 16.0 * 9.0) * 0.5) : 0) + "px";
    document.getElementById('isstgap').style.height = hh;
    document.getElementById('isstgap2').style.height = hh;
    if (!isDocumentInFullScreenMode())
        document.getElementById('map_canvas').style.height = (220.0 / 625.0 * isst_map_w) + "px";
    ref = document.getElementById('cover');
    ref.style.maxWidth = ((isDocumentInFullScreenMode()) ? screen.width : 625) + "px";
